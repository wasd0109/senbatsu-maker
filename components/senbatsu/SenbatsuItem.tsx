...
function SenbatsuItem({ rowIndex, colIndex, member }: SenbatsuItemProps) {
    ...
    if (member) {
        return (
            <div className="relative mx-1 my-2" key={`${rowIndex}-${colIndex}`}>
                ...
                {/* Member card */}
                <div className={`${dragging ? "opacity-50" : ""} ${isDraggedOver ? "bg-gray-500 opacity-30" : ""} border border-gray-300 w-[100px] h-40 relative z-10`} ref={draggableRef}>
